    ...
    <div>
      ...
      <div className="table-responsive">
        {hardwareList && hardwareList.length > 0 ? (
            ...
            <tbody>
              {hardwareList.map((hardware, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/hardware/${hardware.id}`} color="link" size="sm">
                      {hardware.id}
                    </Button>
                  </td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/hardware/${hardware.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                        <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">View</span>
                      </Button>
                      <Button tag={Link} to={`/hardware/${hardware.id}/edit`} color="primary" size="sm" data-cy="entityEditButton">
          ...
          !loading && <div className="alert alert-warning">No Hardware found</div>
        )}
      </div>
    </div>
  );
