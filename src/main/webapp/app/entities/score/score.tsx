          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th>
                  Scorebijleeggebied Leefgebied <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  Hoogtescore Scoresoort <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  Heeft Client <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {scoreList.map((score, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{score.datum}</td>
                  <td>
                    {score.scorebijleeggebiedLeefgebied ? (
                      <Link to={`/leefgebied/${score.scorebijleeggebiedLeefgebied.id}`}>{score.scorebijleeggebiedLeefgebied.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td>
                    {score.hoogtescoreScoresoort ? (
                      <Link to={`/scoresoort/${score.hoogtescoreScoresoort.id}`}>{score.hoogtescoreScoresoort.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td>{score.heeftClient ? <Link to={`/client/${score.heeftClient.id}`}>{score.heeftClient.id}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        onClick={() => (window.location.href = `/score/${score.id}/delete`)}
                        color="danger"
                        size="sm"
