...
/**
 * REST controller for managing {@link nl.ritense.demo.domain.Document}.
 */
@RestController
...
public class DocumentResource {
    ...
    public ResponseEntity<Document> partialUpdateDocument(
        ...
        Optional<Document> result = documentRepository
            ...
            .map(existingDocument -> {
                if (document.getCocumentbeschrijving() != null) {
                    existingDocument.setCocumentbeschrijving(document.getCocumentbeschrijving());
                }
                if (document.getDatumcreatiedocument() != null) {
                    existingDocument.setDatumcreatiedocument(document.getDatumcreatiedocument());
                }
                if (document.getDatumontvangstdocument() != null) {
                    existingDocument.setDatumontvangstdocument(document.getDatumontvangstdocument());
                }
                if (document.getDatumverzendingdocument() != null) {
                    existingDocument.setDatumverzendingdocument(document.getDatumverzendingdocument());
                }
                if (document.getDocumentauteur() != null) {
                    existingDocument.setDocumentauteur(document.getDocumentauteur());
                }
                if (document.getDocumentidentificatie() != null) {
                    existingDocument.setDocumentidentificatie(document.getDocumentidentificatie());
                }
                if (document.getDocumenttitel() != null) {
                    existingDocument.setDocumenttitel(document.getDocumenttitel());
                }
                if (document.getVertrouwelijkaanduiding() != null) {
                    existingDocument.setVertrouwelijkaanduiding(document.getVertrouwelijkaanduiding());
                }
    ...
    public List<Document> getAllDocuments(@RequestParam(name = "filter", required = false) String filter) {
        if ("isvastgelegdinverkeersbesluit-is-null".equals(filter)) {
            log.debug("REST request to get all Documents where isvastgelegdinVerkeersbesluit is null");
            return StreamSupport.stream(documentRepository.findAll().spliterator(), false)
                .filter(document -> document.getIsvastgelegdinVerkeersbesluit() == null)
                .toList();
        }

        if ("isvastgelegdinbesluit-is-null".equals(filter)) {
            log.debug("REST request to get all Documents where isvastgelegdinBesluit is null");
            return StreamSupport.stream(documentRepository.findAll().spliterator(), false)
                .filter(document -> document.getIsvastgelegdinBesluit() == null)
                .toList();
        }
