package nl.ritense.demo.domain;

import jakarta.persistence.*;
import java.io.Serializable;

/**
 * A Entreekaart.
 */
@Entity
@Table(name = "entreekaart")
@SuppressWarnings("common-java:DuplicatedBlocks")
public class Entreekaart implements Serializable {

    private static final long serialVersionUID = 1L;

    @Id
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "sequenceGenerator")
    @SequenceGenerator(name = "sequenceGenerator")
    @Column(name = "id")
    private Long id;

    @Column(name = "rondleiding")
    private String rondleiding;

    // jhipster-needle-entity-add-field - JHipster will add fields here

    public Long getId() {
        return this.id;
    }

    public Entreekaart id(Long id) {
        this.setId(id);
        return this;
    }

    public void setId(Long id) {
        this.id = id;
    }

    public String getRondleiding() {
        return this.rondleiding;
    }

    public Entreekaart rondleiding(String rondleiding) {
        this.setRondleiding(rondleiding);
        return this;
    }

    public void setRondleiding(String rondleiding) {
        this.rondleiding = rondleiding;
    }

    // jhipster-needle-entity-add-getters-setters - JHipster will add getters and setters here

    @Override
    public boolean equals(Object o) {
        if (this == o) {
            return true;
        }
        if (!(o instanceof Entreekaart)) {
            return false;
        }
        return getId() != null && getId().equals(((Entreekaart) o).getId());
    }

    @Override
    public int hashCode() {
        // see https://vladmihalcea.com/how-to-implement-equals-and-hashcode-using-the-jpa-entity-identifier/
        return getClass().hashCode();
    }

    // prettier-ignore
    @Override
    public String toString() {
        return "Entreekaart{" +
            "id=" + getId() +
            ", rondleiding='" + getRondleiding() + "'" +
            "}";
    }
}
